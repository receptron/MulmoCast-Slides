# Markdown → ExtendedScript 変換パイプライン

Issue: #78

## 概要

外部スキルが生成した構造化Markdownを、ExtendedScript形式に変換するパイプライン。
...
- `z.toJSONSchema(extendedScriptSchema)` を毎回実行し最新JSON Schema生成
...
- `extendedScriptSchema.safeParse()` で検証
