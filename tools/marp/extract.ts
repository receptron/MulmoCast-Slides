...
// Render slides to HTML using Marp CLI
function renderSlidesToHTML(markdownPath: string): { htmlPath: string; slideCount: number } {
  ...
  try {
    ...
    console.error('Error: Failed to render Marp presentation');
...
async function captureScreenshots(htmlPath: string, slideCount: number): Promise<void> {
  ...
  try {
    ...
    for (let i = 1; i <= slideCount; i++) {
      // Navigate to specific slide using hash routing
      await page.goto(`${htmlUrl}#${i}`, { waitUntil: 'networkidle2' });
    ...
    console.error('Error: Failed to capture screenshots');
...
async function main() {
  try {
    ...
    // Capture screenshots for image format
    console.log('Capturing screenshots...');
    await captureScreenshots(htmlPath, slideCount);
    console.log(`Captured ${slideCount} screenshots`);
